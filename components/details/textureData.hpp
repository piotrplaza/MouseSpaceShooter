...
struct TextureData
{
	...
	{
		if (std::holds_alternative<std::pair<float*, int>>(loaded.data))
...
private:
	template<typename ColorType>
	float* getRawData(std::vector<ColorType>& data) const
	{
		assert(!data.empty());
		if (data.empty())
			return nullptr;
	...
	float* getRawData(std::pair<std::vector<float>, int>& data) const
	{
		assert(!data.first.empty());
		return data.first.data();
	}

	float* getRawData(std::pair<float*, int>& data) const
	{
		assert(data.first);
		return data.first;
	}

	float* getRawData(std::monostate) const
	{
