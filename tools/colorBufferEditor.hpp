...
namespace Tools
{
	...
	public:
		...
		void updateSubImage(const float* textureSubData, const glm::ivec2& size, const glm::ivec2& offsetPos, int numOfChannels, float spriteAlphaThreshold = 0.0f)
		{
			auto clippedTextureSubData = Tools::GetClippedSubImage(textureSubData, size, offsetPos, res, numOfChannels, operationalBuffer);
			...
			auto processRow = [&](const auto y) {
				const int destIndex = (clippedTextureSubData.offsetPos.y + y) * res.x * numOfDestChannels + clippedTextureSubData.offsetPos.x * numOfDestChannels;
		...
		Tools::SubImageData getSubImage(const glm::ivec2& offsetPos, const glm::ivec2& size, std::vector<float>& operationalBuffer)
		{
			return Tools::GetSubImage(getRawData(), res, offsetPos, size, getNumOfChannels(), operationalBuffer);
		}
