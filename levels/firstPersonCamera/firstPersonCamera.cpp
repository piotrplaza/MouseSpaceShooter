...
namespace Levels
{
	...
	public:
		...
		void createDecorations() const
		{
			...
			Shapes3D::AddSphere(wallsAndInnerSphere, 0.1f, 50, 50, false, [](auto) { return glm::vec4(1.0f); });
			wallsAndInnerSphere.params3D->ambient(0.2f).specular(1.0f).specularFocus(32.0f);

			auto& lamp = staticDecorations.emplace();
			Shapes3D::AddSphere(lamp, 0.05f, 50, 50, false, [](auto) { return glm::vec4(1.0f); }, glm::translate(glm::mat4(1.0f), { 0.0f, 0.5f, 0.0f }));
	...
	void FirstPersonCamera::step()
	{
		...
		const auto positionStep = [&](const float radDirection)
		{
			camera.position.x += std::cos(camera.rotation.y + radDirection) * moveSpeed * physics.frameDuration;
			camera.position.z += std::sin(camera.rotation.y + radDirection) * moveSpeed * physics.frameDuration;
		};
		...
		camera.rotation.x = std::clamp(camera.rotation.x + mouse.getCartesianDelta().y * mouseSensitivity, -glm::half_pi<float>(), glm::half_pi<float>());
		camera.rotation.y = std::fmodf(camera.rotation.y + mouse.getCartesianDelta().x * mouseSensitivity, glm::two_pi<float>());
	}
}
