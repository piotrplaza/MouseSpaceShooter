				...
				if (subData)
				{
					...
					for (auto& [imageDataVariant, offsetPos] : subData->imagesData)
					{
						...
						const auto& [clippedTextureSubData, clippedSize, clippedOffsetPos] =
							Tools::GetClippedSubImage(imageData.getRawData(), imageData.loaded.size, offsetPos,
								texture.loaded.size, imageNumOfChannels, textures.operationalBuffer);
						if (clippedTextureSubData)
						{
							if (subData->updateCallbackF)
								subData->updateCallbackF(clippedTextureSubData, clippedSize, clippedOffsetPos, imageNumOfChannels);
							glTexSubImage2D(GL_TEXTURE_2D, 0, clippedOffsetPos.x, clippedOffsetPos.y, clippedSize.x, clippedSize.y, Tools::GetPixelFormat(imageData.getNumOfChannels()), GL_FLOAT, clippedTextureSubData);
						}
					}
